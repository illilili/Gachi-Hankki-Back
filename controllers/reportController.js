...
exports.reportUser = async (req, res) => {
  ...
  try {
    ...
    if (!receiverNickname || !reason) {
      ...
      return res.status(400).json({ success: false, message: '유효하지 않은 요청입니다. reportedNickname과 reason이 필요합니다.' });
    }

    console.log("Reporting path:", `reports/users/${reportedUid}`); // 추가된 로그
    const reportsRef = db.collection("reports").doc("users").collection(reportedUid);
    ...
    console.log("신고 정보가 Firestore에 저장되었습니다.");

    res.status(201).json({ success: true, message: '상대방이 신고되었습니다.' });
  } catch (error) {
